...
LuaGlobalState::LuaGlobalState() :
	_state(lua::newState())
{
	lua::openDefaultLibs(_state);
	lua::lib::loadDefaults(_state);
}
...
{
	env[lua::lib::name::Import] = luabridge::getGlobal(state, lua::lib::name::Import);
	env[lua::lib::name::OpenLib] = luabridge::getGlobal(state, lua::lib::name::OpenLib);
}
